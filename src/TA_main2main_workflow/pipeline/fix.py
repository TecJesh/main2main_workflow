...
def ai_fix(ctx: WorkflowContext, config: TAConfig, attempt: int = 1) -> WorkflowContext:
    ...
    try:
        result = run_opencode_adapter({"step_id": f"{step_id}-fix-{attempt}", "step_dir": str(step_dir), "fix_dir": str(fix_dir), "ascend_path": str(ascend_path), "triton_path": ctx.triton_ascend_path, "reference_dir": _REF, "mode": "fix", "error_logs": json.dumps(ctx.fix_errors, ensure_ascii=False), "target_commit": ctx.target_commit, "step_index": f"{ctx.current_step+1}/{ctx.total_steps}"})
        log.ai_result(bool(result.modified_files), result.modified_files, (result.step_summary or "")[:500])
        rec = {"step_id": step_id, "attempt": attempt, "fix_type": "build_or_test", "modified_files": result.modified_files, "ai_summary": (result.step_summary or "")[:2000]}
        return ctx.copy_with(fix_attempts=ctx.fix_attempts + [rec])
