...
def merge_upstream_commit(ctx: WorkflowContext, config: TAConfig) -> WorkflowContext:
    ...
    step = ctx.steps[ctx.current_step] if ctx.steps else {"id": "step-0", "end_commit": ctx.target_commit}
    ...
    if config.resume and result_file.exists():
        ...
        return ctx.copy_with(merge_has_conflicts=mr.get("has_conflicts", False),
                             conflict_files=mr.get("conflict_files", []))

    # Abort stale merge if any
    ...
    merge_proc = run_git_no_check(ascend_path, "merge", "--no-ff", "--no-edit", step["end_commit"])

    conflict_files = run_git(ascend_path, "diff", "--name-only", "--diff-filter=U").strip()
    conflict_files = [f for f in conflict_files.splitlines() if f] if conflict_files else []
    has_conflicts = len(conflict_files) > 0

    result = {"target_commit": step["end_commit"], "merge_exit_code": merge_proc.returncode,
              "has_conflicts": has_conflicts, "conflict_files": conflict_files}
    result_file.write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")

    return ctx.copy_with(merge_has_conflicts=has_conflicts, conflict_files=conflict_files)
