...
_CLEANUP_DIRS = ["result_profiling", "__pycache__", ".pytest_cache", ".mypy_cache", "*.egg-info"]
    ...
    for d in _CLEANUP_DIRS:
        for found in repo.rglob(d):
            if found.is_dir() and ".git" not in found.parts:
                try: shutil.rmtree(found, ignore_errors=True); removed_dirs.append(str(found.relative_to(repo)))
                except Exception: pass
    for suffix in _CLEANUP_SUFFIXES:
        for found in repo.rglob(f"*{suffix}"):
            if found.is_file() and ".git" not in found.parts:
                try: found.unlink(); removed_files.append(str(found.relative_to(repo)))
                except Exception: pass
...
def _get_modified_files(repo: Path) -> list[str]:
    ...
    for args in [("diff", "--name-only", "HEAD"), ("diff", "--name-only", "--cached"), ("ls-files", "--others", "--exclude-standard")]:
        r = run_git_no_check(repo, *args)
        if r.stdout.strip(): modified.update(r.stdout.strip().splitlines())
...
def _check_conflict_markers(repo: Path, files: list[str]) -> dict:
    ...
    for fp in files:
        ...
        try: content = full.read_text(encoding="utf-8", errors="replace")
        except Exception: continue
        for lineno, line in enumerate(content.splitlines(), 1):
            for m in _CONFLICT_MARKERS:
                if line.strip().startswith(m):
                    violations.append({"file": fp, "line": lineno, "marker": m})
    return {"name": "conflict_markers", "passed": len(violations) == 0, "violations": violations}
...
def _check_python_syntax(repo: Path, files: list[str]) -> dict:
    ...
    for fp in [f for f in files if f.endswith(".py")]:
        full = repo / fp
        if not full.is_file(): continue
        try: ast.parse(full.read_text(encoding="utf-8"), filename=fp)
        except SyntaxError as e: violations.append({"file": fp, "line": e.lineno or 0, "msg": str(e.msg)})
        except Exception: pass
    return {"name": "python_syntax", "passed": len(violations) == 0, "violations": violations}


def run_pre_ci_check(repo: Path, step_id: str = "") -> dict:
    log.section(f"Pre-CI Check{' — ' + step_id if step_id else ''}")
    try: modified_files = _get_modified_files(repo)
    ...
    if not conflict["passed"]: all_passed = False
    syntax = _check_python_syntax(repo, modified_files)
    log.status(syntax["passed"], syntax.get("detail", ""))
    if not syntax["passed"]: all_passed = False

    result = {"all_passed": all_passed, "checks": [conflict, syntax], "modified_files_count": len(modified_files)}
    (WORKSPACE_DIR / PRE_CI_CHECK_FILE).write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    return result
